...
{
	extern Task tasks[];
    int rq_idx = thiscpu->cpu_task ? thiscpu->cpu_rq.current_index : 0;
    int next_pid;

    int i;
    for (i = 0; i < thiscpu->cpu_rq.total; i++) {
        rq_idx = (rq_idx + 1) % thiscpu->cpu_rq.total;
        next_pid = thiscpu->cpu_rq.runQueue[rq_idx];
        if (tasks[next_pid].state == TASK_RUNNABLE) {
            ...
            thiscpu->cpu_rq.current_index = rq_idx;
