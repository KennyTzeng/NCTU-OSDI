...
setupkvm()
{
    ...
    int i;
    for(i = 0; i < NCPU; i++) {
        boot_map_region(pgdir, KSTACKTOP - i * (KSTKSIZE + KSTKGAP) - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W);
    }

    // MMIO region for local apic
    extern volatile uint32_t *lapic;
    extern physaddr_t lapicaddr;
    boot_map_region(pgdir, lapic, ROUNDUP(4096, PGSIZE), lapicaddr, PTE_W | PTE_PCD | PTE_PWT);
